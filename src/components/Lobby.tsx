...
export function Lobby() {
  ...
  // Calculate expected split based on player count
  ...
  else if (playerCount === 9) { majSize = "5-6"; minSize = "3-4"; } // Variable split
      ...
      <div className="z-10 w-full max-w-[1000px] grid grid-cols-1 lg:grid-cols-12 gap-12">
        ...
        <div className="lg:col-span-7 flex flex-col">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-2xl font-serif text-[var(--color-gold)]">Guests Arriving</h2>
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <span className="font-sans text-[15px] text-[var(--color-ivory)]">
                    {p.name} {p.isMe && <span className="text-[var(--color-ash)] italic">(You)</span>}
                  </span>
                </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-widest flex items-center gap-1 ${
                    p.ready ? "bg-[#2D7A4A]/20 text-[#2D7A4A] border border-[#2D7A4A]/30" : "bg-[var(--color-charcoal-warm)] text-[var(--color-ash)]"
                  }`}>
          ...
          <div className="bg-[var(--color-ballroom)] border border-[var(--color-charcoal-warm)] rounded-lg p-4 flex items-center justify-between">
            <div>
              <p className="text-[10px] uppercase tracking-widest text-[var(--color-ash)] mb-1">Room Invitation</p>
              <p className="font-mono text-lg text-[var(--color-ivory)] tracking-widest">{gameState.roomId}</p>
            </div>
            <button 
              onClick={() => {
        ...
        <div className="lg:col-span-5 flex flex-col">
          <div className="bg-[var(--color-ballroom)] border border-[var(--color-charcoal-warm)] rounded-xl p-8 flex flex-col h-full">
            <h3 className="text-xl font-serif text-[var(--color-ivory)] mb-6 border-b border-[var(--color-charcoal-warm)] pb-4">The Masquerade</h3>
            
            <div className="space-y-4 mb-auto">
              <div className="flex justify-between items-center text-sm">
                <span className="text-[var(--color-ash)]">Alliance Split</span>
                <span className="text-[var(--color-gold)] font-bold">{splitText}</span>
              </div>
              <div className="flex justify-between items-center text-sm">
                <span className="text-[var(--color-ash)]">Card Hand</span>
                <span className="text-[var(--color-ivory-antique)]">10 Cards</span>
              </div>
              <div className="flex justify-between items-center text-sm">
                <span className="text-[var(--color-ash)]">Elimination</span>
                <span className="text-[var(--color-ivory-antique)]">Mandatory</span>
              </div>
            </div>
