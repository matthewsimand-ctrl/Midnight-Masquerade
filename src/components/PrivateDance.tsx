...
export function PrivateDance() {
  ...
  const partnerId = me
    ? gameState?.dancePairs[me.id] ||
      Object.keys(gameState?.dancePairs || {}).find(
        (id) => gameState?.dancePairs[id] === me.id,
      )
    : null;
  ...
  if (me?.isEliminated) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-8 relative overflow-hidden">
        ...
        <h2 className="text-2xl text-[var(--color-ash)] font-serif mb-4 uppercase tracking-widest z-10">
          You are eliminated.
        </h2>
        <p className="text-[var(--color-ivory-antique)] z-10">
          The private dances are happening behind closed doors...
        </p>
        ...
        {isHost && !gameState.allPairsShared && (
          <p className="mt-12 text-[var(--color-gold)] font-serif italic z-10">
            Waiting for pairs to finish...
          </p>
        )}
      </div>
    );
  ...
  if (!partnerId) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-8 relative overflow-hidden">
        <div className="velvet-texture"></div>
        <h2 className="text-2xl text-[var(--color-ash)] font-serif mb-4 uppercase tracking-widest z-10">
          You are resting this dance.
        </h2>
        <p className="text-[var(--color-ivory-antique)] z-10">
          Observe the others as they share their secrets...
        </p>
        {isHost && gameState.allPairsShared && (
          <button
            onClick={() => advancePhase()}
      ...
      <div className="w-full h-full flex flex-col">
        {card.type === "Image" ? (
          ...
          <>
            ...
            <div className="flex-1 flex items-center justify-center p-3 sm:p-6 bg-[var(--color-ballroom)] text-[var(--color-ivory)] font-serif italic text-sm sm:text-xl text-center break-words border-y border-[var(--color-gold)]/30 my-2 sm:my-4 mx-1 sm:mx-2">
              {card.content}
            </div>
          </>
        )}
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center bg-[var(--color-midnight)] relative overflow-hidden px-3 sm:px-0">
      <div
        className={`absolute inset-y-0 left-0 w-1/2 bg-[var(--color-burgundy)] border-r-4 border-[var(--color-gold)] z-50 transition-transform duration-[1200ms] ease-[cubic-bezier(.4,0,.2,1)] ${
      ...
      <div
        className={`absolute inset-y-0 right-0 w-1/2 bg-[var(--color-burgundy)] border-l-4 border-[var(--color-gold)] z-50 transition-transform duration-[1200ms] ease-[cubic-bezier(.4,0,.2,1)] ${
          curtainsOpen ? "translate-x-full" : "translate-x-0"
        }`}
      >
        <div className="velvet-texture opacity-20"></div>
      ...
      <div className="absolute top-3 sm:top-6 left-1/2 -translate-x-1/2 z-20 bg-[var(--color-velvet)] border border-[var(--color-gold)]/30 rounded-full px-4 sm:px-8 py-2 shadow-[0_0_20px_rgba(212,175,55,0.15)] max-w-[90vw]">
        <p className="text-[var(--color-gold)] font-serif text-sm sm:text-lg tracking-[0.15em] sm:tracking-widest text-center truncate">
          "{gameState.currentMotif}"
        </p>
      </div>

      <div className="w-full h-full flex relative z-10 pt-20 sm:pt-24 pb-28 sm:pb-24">
        {!mySharedCard ? (
          <div className="w-full flex flex-col items-center px-2 sm:px-12 relative z-10">
            <div className="flex flex-col items-center mb-8">
              <h3 className="text-xl font-serif text-[var(--color-gold)] uppercase tracking-widest">
                Your Hand
              </h3>
                  ...
                  <button
                    ...
                    className={`aspect-[2/3] rounded-md border transition-all overflow-hidden flex flex-col relative ${
                      selectedCardId === card.id
                        ? "border-[var(--color-gold)] shadow-[0_0_15px_rgba(212,175,55,0.4)] sm:-translate-y-2"
                        : "border-[var(--color-charcoal-rich)] bg-[var(--color-velvet)] hover:border-[var(--color-gold)]/50 sm:hover:-translate-y-1"
                    }`}
                  >
                    {card.type === "Image" ? (
                      <>
              ...
              {selectedCardId && (
                <div className="flex justify-center animate-in slide-in-from-bottom-4">
                  <button
                    ...
                    className="px-6 sm:px-8 py-3 rounded bg-gradient-to-br from-[var(--color-gold)] to-[var(--color-gold-light)] text-[var(--color-midnight)] font-serif font-bold uppercase tracking-[0.15em] sm:tracking-widest text-sm shadow-[0_0_30px_rgba(212,175,55,0.6)] hover:scale-105 transition-transform animate-pulse"
                  >
                    Share This Card
                  </button>
                </div>
          ...
          <div className="w-full h-full flex flex-col sm:flex-row gap-8 sm:gap-0 items-center">
            ...
            <div className="w-full sm:w-1/2 flex flex-col items-center px-4 sm:px-12 relative z-10">
              <div className="flex flex-col items-center mb-8">
                <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-full bg-[var(--color-charcoal-rich)] border-2 border-[var(--color-gold)]/30 flex items-center justify-center text-4xl mb-4 animate-float overflow-hidden">
            ...
            <div className="w-full sm:w-1/2 flex flex-col items-center px-4 sm:px-12 relative z-10">
              <div className="flex flex-col items-center mb-8">
                <div
                  className="w-20 h-20 sm:w-24 sm:h-24 rounded-full bg-[var(--color-charcoal-rich)] border-2 border-[var(--color-gold)]/30 flex items-center justify-center text-4xl mb-4 animate-float overflow-hidden"
                ...
                <h3 className="text-xl font-serif text-[var(--color-gold)] uppercase tracking-widest text-center">
                  {partner?.name}'s Card
                </h3>
              </div>
              ...
              <div className="w-36 sm:w-48 aspect-[2/3] relative">
                ...
                >
                  <div className="w-28 sm:w-32 h-40 sm:h-44 border border-[var(--color-gold)]/30 rounded flex items-center justify-center overflow-hidden">
                    {partner?.avatar?.startsWith("/") ? (
                ...
                {!partnerSharedCard && (
                  <div className="absolute inset-0 rounded-md border border-[var(--color-charcoal-rich)] bg-[var(--color-velvet)] flex flex-col items-center justify-center gap-3">
                    ...
                    <p className="text-[var(--color-ash)] text-xs font-serif italic animate-pulse">
                      Waiting...
                    </p>
                  </div>
                )}
      ...
      <div className="absolute bottom-0 left-0 right-0 h-24 bg-gradient-to-t from-[var(--color-midnight)] to-transparent z-20 flex items-center justify-center px-3">
        {isHost && gameState.allPairsShared && (
          <button
            onClick={() => advancePhase()}
            className="px-6 sm:px-8 py-3 sm:py-4 rounded bg-gradient-to-br from-[var(--color-crimson)] to-[var(--color-crimson-active)] text-[var(--color-ivory)] font-serif font-bold uppercase tracking-[0.15em] sm:tracking-widest text-sm sm:text-base shadow-[0_0_20px_rgba(156,28,43,0.4)] hover:scale-105 transition-transform animate-in slide-in-from-bottom-8"
          >
            Conclude the Dance
          </button>
        )}
        {isHost && !gameState.allPairsShared && mySharedCard && partnerSharedCard && (
          <p className="text-[var(--color-gold)] font-serif italic animate-pulse text-center text-sm sm:text-base">
            Waiting for other pairs to finish...
          </p>
        )}
      </div>
    </div>
