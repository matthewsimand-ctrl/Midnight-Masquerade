        ...
        {(mode === "create" || mode === "join") && (
          <div className="w-full bg-[var(--color-ballroom)] border border-[var(--color-charcoal-warm)] p-8 rounded-lg shadow-2xl">
            <div className="flex justify-between mb-8 border-b border-[var(--color-charcoal-warm)]">
              <button 
                className={`pb-2 font-serif uppercase tracking-widest ${mode === "create" ? "text-[var(--color-gold)] border-b-2 border-[var(--color-gold)]" : "text-[var(--color-ash)]"}`}
                onClick={() => setMode("create")}
              >
                Create Room
              </button>
              <button 
                className={`pb-2 font-serif uppercase tracking-widest ${mode === "join" ? "text-[var(--color-gold)] border-b-2 border-[var(--color-gold)]" : "text-[var(--color-ash)]"}`}
                onClick={() => setMode("join")}
              >
                Join Room
              </button>
            ...
            <div className="space-y-6">
              <div>
                <label className="block text-[var(--color-ash)] text-sm mb-2">Your Name</label>
                <input
                  type="text"
              ...
              {mode === "join" && (
                <div>
                  <label className="block text-[var(--color-ash)] text-sm mb-2">Room Code</label>
                  <input
                    type="text"
                    placeholder="MQN-4A7"
                    value={roomId}
...
export default function App() {
  ...
  if (!gameState) return <div className="min-h-screen bg-[var(--color-midnight)] text-[var(--color-ivory)] flex items-center justify-center">Loading...</div>;
  ...
  const renderPhase = () => {
    switch (gameState.phase) {
      ...
      case "MotifReveal": return <MotifReveal />;
      case "PrivateDance": return <PrivateDance />;
      case "GossipSalon": return <GossipSalon />;
      case "EliminationVote": return <EliminationVote />;
  ...
  return (
    <div className="min-h-screen bg-[var(--color-midnight)] text-[var(--color-ivory)] font-sans flex overflow-hidden">
      ...
      {gameState.phase !== "Lobby" && gameState.phase !== "Dealing" && <Journal />}

      {/* Hand Drawer */}
      {gameState.phase !== "Lobby" && gameState.phase !== "Dealing" && me && me.hand && me.hand.length > 0 && (
        <>
          <button
          ...
          <div className={`fixed bottom-0 left-0 right-0 bg-[var(--color-ballroom)] border-t border-[var(--color-charcoal-warm)] p-6 z-40 transition-transform duration-300 ease-in-out ${
            showHand ? "translate-y-0" : "translate-y-full"
          }`}>
            <div className="max-w-6xl mx-auto">
              <h3 className="text-xl font-serif text-[var(--color-gold)] mb-4">Your Hand</h3>
              <div className="flex gap-6 overflow-x-auto pb-4">
                {me.hand.map(card => (
                  <div key={card.id} className="w-48 flex-shrink-0 aspect-[2/3] rounded-md border border-[var(--color-charcoal-rich)] bg-[var(--color-velvet)] overflow-hidden flex flex-col shadow-lg">
                    {card.type === "Image" ? (
                      <>
